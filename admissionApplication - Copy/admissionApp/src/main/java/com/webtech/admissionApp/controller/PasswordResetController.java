...
public class PasswordResetController {
    ...
    public String handleForgotPassword(@RequestParam("email") String email, Model model) {
        ...
        if (!userService.doesEmailExist(email)) {
            ...
            return "reset-password"; // Redirect back to forgot password page
        }
    }
    ...
    // }
    @PostMapping("/reset-password")
    public String handlePasswordReset(@RequestParam("newPassword") String newPassword,
                                    @RequestParam("confirmNewPassword") String confirmNewPassword,
                                    Model model) {
        ...
        if (!newPassword.equals(confirmNewPassword)) {
            model.addAttribute("error", "Passwords do not match. Please try again.");
            return "login"; // Show reset password page again with error
        }

        // Check if new password is "2002" (you can remove this if not needed) 
        if (newPassword.equals("2002")) {
